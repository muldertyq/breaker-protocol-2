...
namespace BreakerProtocol.Tools.ModuleEditor.Viewport
{
	public partial class ModuleGridCanvas
	{
		public override void _Draw()
		{
			...
			int gridW = CurrentModule.Width;
			int gridH = CurrentModule.Height;
			Vector2 origin = _canvasPan;
			Vector2 totalPx = new Vector2(gridW * GridUnitPixels, gridH * GridUnitPixels) * _canvasZoom;
			...
			if (hp != null && hp.DroneSprite != _cachedDronePath)
			{
			...
			// 5. 🛸 机库跑道起点待命无人机渲染 (由 ShowDroneOnRunway 控制)
			if (hp != null && hp.Runways != null)
			{
				if (hp.ShowDroneOnRunway)
				{
					for (int i = 0; i < hp.Runways.Length; i++)
					{
						// 非测试模式下常驻显示；测试模式下仅在跑道就绪时显示
						bool isDocked = !_turretHandler.IsTestFiringMode || (i < _runtimeRunways.Count && _runtimeRunways[i].IsReady);
						if (isDocked)
						{
							var rw = hp.Runways[i];
							Vector2 startScreen = origin + new Vector2(rw.StartOffsetX, rw.StartOffsetY) * _canvasZoom;
							Vector2 dir = new Vector2(rw.ExitOffsetX - rw.StartOffsetX, rw.ExitOffsetY - rw.StartOffsetY).Normalized();
							if (dir.LengthSquared() < 0.001f) dir = new Vector2(0, -1);
							float angle = Mathf.Atan2(dir.Y, dir.X) + Mathf.Pi * 0.5f;

							DrawDroneSprite(startScreen, angle, hp.DroneWidth * _canvasZoom, hp.DroneLength * _canvasZoom, DroneTexture);
						}
					}
				}
			}
			...
			if (wp != null && wp.DeliveryType == "Missile")
			{
				...
				if (wp.Bays != null)
				{
					...
					{
						float openProg = _runtimeBays.TryGetValue(bay.BayId, out var bs) ? bs.OpenProgress : 0.0f;
						Vector2 bayScreenPos = origin + new Vector2(bay.OffsetX, bay.OffsetY) * _canvasZoom;
						float halfW = bay.Width * 0.5f * _canvasZoom;
						float bayH = bay.Height * _canvasZoom;
						Texture2D? hatchTex = LoadTextureAuto(bay.CustomHatchSprite);

						if (bay.AnimationType == "InstantHide")
						{
							if (openProg <= 0.01f)
							{
								Rect2 fullDoor = new(bayScreenPos.X - halfW, bayScreenPos.Y - bayH * 0.5f, bay.Width * _canvasZoom, bayH);
								if (hatchTex != null) DrawTextureRect(hatchTex, fullDoor, false);
								else
								{
									DrawRect(fullDoor, new Color(0.18f, 0.22f, 0.28f, 0.98f), true);
									DrawRect(fullDoor, new Color(0.45f, 0.55f, 0.70f), false, 1.0f);
									DrawLine(fullDoor.Position, fullDoor.End, new Color(0.3f, 0.35f, 0.45f), 1.0f);
								}
							}
						}
						else if (bay.AnimationType == "Split")
						{
							float slideOffset = halfW * openProg;
							Rect2 leftDoor = new(bayScreenPos.X - halfW - slideOffset, bayScreenPos.Y - bayH * 0.5f, halfW, bayH);
							Rect2 rightDoor = new(bayScreenPos.X + slideOffset, bayScreenPos.Y - bayH * 0.5f, halfW, bayH);

							DrawRect(leftDoor, new Color(0.18f, 0.22f, 0.28f, 0.95f), true);
							DrawRect(leftDoor, new Color(0.4f, 0.5f, 0.65f), false, 1.0f);
							DrawRect(rightDoor, new Color(0.18f, 0.22f, 0.28f, 0.95f), true);
							DrawRect(rightDoor, new Color(0.4f, 0.5f, 0.65f), false, 1.0f);
						}
						else if (bay.AnimationType == "Fade")
						{
							float alpha = Mathf.Clamp(1.0f - openProg, 0.0f, 1.0f);
							if (alpha > 0.01f)
							{
								Rect2 fullDoor = new(bayScreenPos.X - halfW, bayScreenPos.Y - bayH * 0.5f, bay.Width * _canvasZoom, bayH);
								DrawRect(fullDoor, new Color(0.18f, 0.22f, 0.28f, 0.95f * alpha), true);
								DrawRect(fullDoor, new Color(0.45f, 0.55f, 0.70f, alpha), false, 1.0f);
							}
						}
						else
						{
							float slideOffset = bay.Width * _canvasZoom * openProg;
							Rect2 fullDoor = new(bayScreenPos.X - halfW + slideOffset, bayScreenPos.Y - bayH * 0.5f, bay.Width * _canvasZoom, bayH);
							DrawRect(fullDoor, new Color(0.18f, 0.22f, 0.28f, 0.95f), true);
							DrawRect(fullDoor, new Color(0.4f, 0.5f, 0.65f), false, 1.0f);
						}
					}
				}
			...
			// 7. 🛸 运行中/滑跑起飞中的无人机
			foreach (var d in _demoDrones)
			{
				Vector2 dScreen = origin + d.Pos * _canvasZoom;

				// 滑跑阶段绘制微弱引导气流，飞行阶段绘制主等离子尾焰
				if (d.Stage == DroneFlightStage.Airborne)
				{
					for (int i = 0; i < d.Trail.Count; i++)
					{
						float trailAlpha = (1.0f - (float)i / d.Trail.Count) * 0.35f;
						Vector2 tScreen = origin + d.Trail[i] * _canvasZoom;
						DrawCircle(tScreen, (d.Width * 0.25f + i * 0.4f) * _canvasZoom, new Color(0.2f, 0.85f, 1.0f, trailAlpha));
					}

					Vector2 moveDir = d.Vel.Normalized();
					Vector2 tailScreen = dScreen - moveDir * (d.Length * 0.45f * _canvasZoom);
					DrawCircle(tailScreen, d.Width * 0.35f * _canvasZoom, new Color(0.2f, 0.85f, 1.0f, 0.85f));
					DrawCircle(tailScreen, d.Width * 0.18f * _canvasZoom, Colors.White);
				}
				else
				{
					// 跑道导轨滑跑指示光标
					DrawCircle(dScreen, d.Width * 0.2f * _canvasZoom, new Color(0.35f, 1.0f, 0.65f, 0.6f));
				}

				DrawDroneSprite(dScreen, d.AngleRad, d.Width * _canvasZoom, d.Length * _canvasZoom, d.Tex);
			}
		...
		private void DrawDroneSprite(Vector2 centerScreen, float angleRad, float w, float h, Texture2D? tex)
		{
			...
			{
				Color bodyColor = new(0.35f, 0.85f, 0.95f);
				Vector2[] wings = { new(0, -h * 0.5f), new(w * 0.5f, h * 0.4f), new(0, h * 0.2f), new(-w * 0.5f, h * 0.4f) };
				DrawColoredPolygon(wings, bodyColor);
				DrawPolyline(wings, Colors.White, 1.2f);
			}
